...
drop policy if exists "Users and project owners can request/join projects" on project_members;
drop policy if exists "Project owners and admins can update project members" on project_members;
drop policy if exists "Project owners and admins can remove project members" on project_members;
drop policy if exists "Projects select policy - public and owned projects" on projects;
drop policy if exists "Approved project members can view project details" on projects;
...
create policy "Projects select policy - public and owned projects" on projects
  for select using (
    ...
    -- Admins and staff can see all projects
    exists (
      select 1 from profiles
      where id = auth.uid() and role in ('admin', 'staff')
    ) or
    -- Cluster members can view cluster projects (breaks the recursion chain)
    (
      type = 'cluster' and
